...
export const fetchProjects = (categoryId) => {
    return (dispatch) => {
        ...
        if(categoryId != null) {
            parameter = "/posts?categories=" + categoryId + "&per_page=100&status=publish";
        }

        dispatch(fetchProjectsRequest());
        axios.get(gv.BASE_API_URL + gv.API_SEGMENT + parameter + "&_embed=1")
            .then(res => {
                ...
                projects.forEach((project, i) => {
                    ...
                    categories.forEach((cat, j) => {
                        if(project.categories.includes(cat.id) && ![430, 109, 1140].includes(cat.id)) {
...
export const fetchProject = (projectSlug) => {
    return (dispatch) => {
        console.log("Fetch Project Working");
        dispatch(fetchProjectRequest);
        axios.get(gv.BASE_API_URL + gv.API_SEGMENT + gv.SINGLE_ENDPOINT + "?slug=" + projectSlug + "&_embed=1&acf_format=standard")
            .then(res => {
                console.log("Case Study", res);
                let project = res.data[0];
                // let categoryId = 11;

                // project.categories

                axios.get(gv.BASE_API_URL + gv.API_SEGMENT + "/posts?categories=430,1140&per_page=3&status=publish&orderBy=date&_embed=1&acf_format=standard")
                    .then(listRes => {
